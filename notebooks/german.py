...
X_train0, X_test0,X_train, X_test, y_train, y_test, S_train, S_test, column_names = LOAD_DATA_TRAIN_TEST('german')
...
for tau in TAUS:
    for lambda_ in LAMBDAS:
        for i in range(N_RUNS):
            ...
            if sys.argv[4] == "ROAD":
                model = ROAD(learning_rate=0.0001,batch_size= 128,
            ...
            gr_rdm = get_subgroups_random(X_test0, S_test, n_groups=5)
    ...
    df_out.to_csv("./../results_german/" + sys.argv[5] + "_DP.csv")
    df_out_rdm = pd.DataFrame(scores_rdm)
    df_out_rdm.to_csv("./../results_german/" + sys.argv[5] + "_DP_rdm.csv")

        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(batch_no):
                ...
                for l in range(nb_iter_s):
                    ...
                    Spred_var = self.m_NN_s(Ypred_var0,y_var)
                    lossS = torch.mean(criterion2(Spred_var, s_var))
                    lossS.backward()
                    self.optimizer_s.step()

                Spred_var0 = self.m_NN_s(Ypred_var0,y_var).detach()

                # Learning Ratio  Apred_var with nb_iter_r iterations
                for m in range(nb_iter_r):
                    self.optimizer_a.zero_grad()
                    Apred_var = self.m_NN_a(x_var_s_var,y_var)/torch.mean(self.m_NN_a(x_var_s_var,y_var))
                    Apred_var[maskS1] = self.m_NN_a(x_var_s_var,y_var)[maskS1]/torch.mean(self.m_NN_a(x_var_s_var,y_var)[maskS1])
                    Apred_var[maskS0] = self.m_NN_a(x_var_s_var,y_var)[maskS0]/torch.mean(self.m_NN_a(x_var_s_var,y_var)[maskS0])
                ...
                Spred_var = self.m_NN_s(Ypred_var,y_var)

...
class BROAD(object):
    """
    BROAD Algorithm
    NonParametric solution (weights on the adversary loss = Boltzmann distribution)
    """
    ...
    def __init__(self, learning_rate, batch_size, lamb, lambda_r,
        ...
        self.name = "BROAD_" + str(lamb) + "_" + str(lambda_r) + "_" + name

        self.GPU = GPU

    def fit(self, X_train, y_train, S_train, X_test=None, y_test=None, S_test=None, plot_losses=False, nb_iter_r = 50):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i in range(batch_no):
                ...
                Apred_var = torch.exp(-criterion2(Spred_var, s_var)/self.lambda_r)/torch.mean(torch.exp(-criterion2(Spred_var, s_var)/self.lambda_r))
                Apred_var[maskS1] = torch.exp(-criterion2(Spred_var[maskS1], s_var[maskS1])/self.lambda_r)/torch.mean(torch.exp(-criterion2(Spred_var[maskS1], s_var[maskS1])/self.lambda_r))
                Apred_var[maskS0] = torch.exp(-criterion2(Spred_var[maskS0], s_var[maskS0])/self.lambda_r)/torch.mean(torch.exp(-criterion2(Spred_var[maskS0], s_var[maskS0])/self.lambda_r))
                
                Apred_var = Apred_var.detach()
